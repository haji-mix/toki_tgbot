    ...
    execute: async ({ chat, args, userId }) => {
      ...
      try {
        ...
        global.replyCallbacks.set(sentMessage.message_id, async (replyMsg) => {
          if (replyMsg.text) {
            const newResponse = await gpt4o(replyMsg.text, userId);
            const newSentMessage = await chat.reply(newResponse);
            
            global.replyCallbacks.set(newSentMessage.message_id, async (newReplyMsg) => {
              if (newReplyMsg.text) {
                const anotherResponse = await gpt4o(newReplyMsg.text, userId);
                await chat.reply(anotherResponse);
              }
            });
          }
        });
